...
def config():
    ...
    click.echo(f"Jira API Token: {'*' * len(jira_config.password) if jira_config.password else 'Not set'}")
...
def validate(csv_file):
    ...
    try:
        ...
        if processor.validate_issue_groups(issue_groups):
            click.echo(f"✅ Issue groups are valid. Found {len(issue_groups)} issue groups.")
        else:
