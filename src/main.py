        ...
        for row in rows:
            if row.has_subtask():
                try:
                    ...
                    logger.info(f"Created subtask {subtask['key']}: {row.subtask_summary}")
...
def main(csv_file_path: Optional[str] = None) -> bool:
    ...
    try:
        ...
        for issue_id, group_rows in issue_groups.items():
            logger.info(f"Processing issue group {issue_id} with {len(group_rows)} rows")
        ...
        for result in all_results:
            if result['main_issue']:
                logger.info(f"Issue {result['main_issue']['key']}: {result['main_issue']['summary']}")
