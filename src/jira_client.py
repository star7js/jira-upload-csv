...
class JiraClient:
    ...
    def _make_request_with_retry(self, operation: str, *args, **kwargs) -> Dict[str, Any]:
        """
        ...
        for attempt in range(self.retry_attempts):
            try:
                logger.debug(f"Attempting {operation} (attempt {attempt + 1}/{self.retry_attempts})")
    ...
    def create_subtask(self, subtask_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully created subtask: {response.get('key', 'Unknown')}")
            return response
